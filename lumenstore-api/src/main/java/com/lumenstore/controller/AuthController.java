...
import com.lumenstore.repository.IRefreshTokenRepository;
...
public class AuthController {
    ...
    public ResponseEntity<?> register(@Valid @RequestBody RegisterRequestDTO request) {
            ...
            if (existing.getUsername().equalsIgnoreCase(request.getUsername())) {
                return ResponseEntity.badRequest().body(
                        new ErrorResponseDTO("El nombre de usuario ya está en uso.",
                                List.of(new ErrorResponseDTO.FieldErrorDTO("username",
                                        "El nombre de usuario ya está en uso."))));
            }
            if (existing.getEmail().equalsIgnoreCase(request.getEmail())) {
                return ResponseEntity.badRequest().body(
                        new ErrorResponseDTO("El correo electrónico ya está en uso.",
                                List.of(new ErrorResponseDTO.FieldErrorDTO("email",
        ...
        try {
            ...
            log.warn("Duplicate registration attempt: {}", request.getUsername());
            return ResponseEntity.badRequest().body(
                    new ErrorResponseDTO("El nombre de usuario o correo ya está en uso.", List.of(
                            new ErrorResponseDTO.FieldErrorDTO("username",
        ...
        List<String> roles = nuevoUsuario.getRoles() != null
                ? nuevoUsuario.getRoles().stream().map(r -> r.getName()).toList()
                : List.of();
    ...
    public ResponseEntity<?> login(@Valid @RequestBody LoginRequestDTO request) {
        ...
        Cliente cliente = clienteRepository.findByUser(usuario).orElse(null);

        List<String> roles = usuario.getRoles() != null
                ? usuario.getRoles().stream().map(r -> r.getName()).toList()
                : List.of();
    ...
    public ResponseEntity<?> logout(
        ...
        try {
            ...
            return ResponseEntity.ok(Map.of("message", "No se proporcionó token, sesión cerrada localmente"));
        } catch (Exception ex) {
